...
{                                             // **Solar_System**:  Your Assingment's Scene.
  constructor()
    {                  // constructor(): Scenes begin by populating initial values like the Shapes and Materials they'll need.
      ...
      this.shapes.ball_5.arrays.texture_coord.forEach(coord => {coord[0]*=5,coord[1]*=5 })
                   ...
                   plastic_stars: new Material( texture_shader_2,    
                     ...
                     metal_earth: new Material( texture_shader_2,    
                                    { texture: new Texture( "assets/earth.gif" ),
                                      ambient: 0, diffusivity: .9, specularity: .2, color: Color.of( .4,.4,.4,1 ) } ),
                      black_hole: new Material( black_hole_shader ),
                             sun: new Material( phong_shader, { ambient: 1, color: Color.of( 1,1,0,1 ) } ),
                        planet_1: new Material( phong_shader, 
                        ...
                        /*planet_4: new Material( texture_shader_2,    
                                    { texture: new Texture( "assets/bricks.png" ),
                                      ambient: 0, diffusivity: 1, specularity: 1, color: Color.of( .4,.4,.4,1 ) }, smoothness:40 ),
                          moon_1: new Material( phong_shader,    
                                    { ambient: 0, diffusivity: 1, specularity: 1, color: Color.of( 1, 1, 1) } )*/
  ...
  make_control_panel()
    {                                 // make_control_panel(): Sets up a panel of interactive HTML elements, including
     ...
     this.key_triggered_button( "Lights on/off", [ "l" ], () => this.lights_on = !this.lights_on ); //should this only be working when you use the key?
  ...
  display( context, program_state )
    {                                                // display():  Called once per frame of animation.  For each shape that you want to
      ...
      **********************************/         
      ...
      let sun = model_transform.copy();
      sun = sun.times( Mat4.scale ([ sun_size,   sun_size, sun_size ]) );
     ...
     this.camera_teleporter.cameras.push(Mat4.inverse (  
                                          planet_1.times(Mat4.rotation(t, Vec.of(0,-1,0)) )
                                                  .times(Mat4.translation([  0,0,4 ])  )
                                                        ));
      this.camera_teleporter.cameras.push(Mat4.inverse (  
                                          planet_2.times(Mat4.rotation(.8*t, Vec.of(0,-1,0)) )
                                                  .times(Mat4.translation([  0,0,4 ])  )
                                                        ));
      this.camera_teleporter.cameras.push(Mat4.inverse (  
